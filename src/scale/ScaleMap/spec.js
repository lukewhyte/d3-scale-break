...
test('ScaleMap.getDomain and ScaleMap.getAllDomains should return respective domains', t => {
...
test('ScaleMap.setDomains should notify listeners', t => {
    const scaleMap = ScaleMap([[-40, 90], [91, 800], [801, 3960]], [0,100]);
...
test('ScaleMap.normalize calculates input\'s position between domain', t => {
...
test('ScaleMap.setNormalizer updates normalizer after ScaleMap.setDomains runs', t => {
...
test('ScaleMap.setScaleRange maps subdomain values to correct position in range and returns subrange', t => {
    let scaleMap = ScaleMap([[0, 100], [101, 800], [801, 1000]], [0,100]);
    let result = scaleMap.setScaleRange(0);
    let expected = [0, 10];
    t.deepEqual(result, expected);

    scaleMap = ScaleMap([[100,50], [49,0]], [10,0]);
    result = scaleMap.setScaleRange(0);
    expected = [10,5];
    t.deepEqual(result, expected);
...
test('ScaleMap.setRange should modify the range and the system should adjust', t => {
    let scaleMap = ScaleMap([[100,50], [49,0]], [-50,100]);
    scaleMap.setRange([0, 100]);

    let result = scaleMap.getRange();
    let expected = [0,100];
    t.deepEqual(result, expected);
...
test('ScaleMap.setRange should notify listeners', t => {
    const scaleMap = ScaleMap([[-40, 90], [91, 800], [801, 3960]], [0,100]);
    scaleMap.testOne = () => scaleMap.booya = () => 'Booya!!';
    scaleMap.testTwo = () => scaleMap.wamp = () => 'Wamp wamp';
...
test('ScaleMap.setScales should set and refresh scales list and handle dynamic domains/ranges', t => {
...
test('ScaleMap.setScopes and ScaleMap.getScopes should manage val range and respond to domain changes', t => {
